...
def start_postgres(expdef, id, ncpus):
    ...
    print("\n")
    print("[start_postgres] fartfile", fartfile)
    print("[start_postgres] dbpath", dbpath)

    cmd = "initdb " + dbpath + " -E utf8 -A trust"

    print("\n")
    print("[start_postgres]", cmd)
    ...
    cmd = "createdb -h " + dbhost + " -p " + str(dbstartport + id) + " " + temp_db_name
    ...
    if 'def_db_sql_script' in expdef:
        cmd = "psql -p %d %s -f %s" % (dbstartport + id, temp_db_name, expdef['def_db_sql_script'])
        run_cmd(cmd, fartfile, fartfile)
    ...
    for e in x['defs']:
        ...
        for rep in range(repeat):
                ...
                for s in range(nprocesses):
                    ...
                    if usedb and s < ndbs:
                        if e['def_db_type'] == 'berkeleydb':
                            servercmd = servercmd + \
                                " --berkeleydb --dbname="+temp_db_name+"_" + str(s) + \
                                " --dbenvpath=" + dbenvpath
                        elif e['def_db_type'] == 'postgres':
                            servercmd = servercmd + \
                                " --postgres --dbname="+temp_db_name+ \
            ...
            if loaddb and dbmonitor and e['def_db_type'] == 'postgres':
                ...
                procs = []
                for s in range(ndbs):
                    archfile = os.path.join(dbarchive, temp_db_name+"_" + str(s))
                    cmd = "pg_restore -p " + str(dbport + s) + " -d "+temp_db_name+" -a " + archfile
            ...
            if dumpdb and dbmonitor and e['def_db_type'] == 'postgres':
                ...
                for s in range(ndbs):
                    ...
                    cmd = "pg_dump -p " + str(dbport + s) + " -f " + archfile + " -a -F d "+temp_db_name
